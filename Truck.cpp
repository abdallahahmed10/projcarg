...
bool Truck::AddCargoToList(Cargos* c)
{
	if (capacity != 0)
	{
		List_Cargos->enqueue(c, -(c->getdelivery_dist())/*(1 / c->getdelivery_dist()) * 10000*/);     //this so that the cargo with the least distance is placed first
		count_cargos++;
		//capacity--;
		return true;
	}
...
bool Truck::CheckIfCargoListIsEmpty()
{
	return (List_Cargos->isEmpty());
}

void Truck::Calc_CDT_ForCargos()
{
	Cargos* cargo;
	LinkedQueue<Cargos*>* temp = new LinkedQueue<Cargos*>;
	while (List_Cargos->dequeue(cargo))
	{
		int CDTD;
		int CDTH;
		CDTD = (movingday);// + ((cargo->getdelivery_dist()) / (speed ))+ cargo->getloadtime());
		CDTH = (movinghour + ((cargo->getdelivery_dist()) / (speed)) + cargo->getloadtime());
		cargo->SetCDT_Day(CDTD);
		cargo->SetCDT_Hour(CDTH);
		temp->enqueue(cargo);
	}
	while (temp->dequeue(cargo))
	{
		int time = cargo->GetCDT_Day() * 24 + cargo->GetCDT_Hour();
		List_Cargos->enqueue(cargo, -time);
	}
	delete temp;
}

bool Truck::PeekCargoList(Cargos*& c)
{
	if (List_Cargos->isEmpty())
		return false;
	 List_Cargos->peekFront(c);
	 return true;
}